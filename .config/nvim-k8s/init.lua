-- ~/.config/nvim-k8s/init.lua
-- Core settings + plugin manager bootstrap.
--
-- Stripped variant of ~/.config/nvim for the k3s control devcontainer.
-- Selected with NVIM_APPNAME=nvim-k8s (set in the container's shell), which
-- makes nvim read this dir instead of ~/.config/nvim.
--
-- Differences from the full config:
--   - no treesitter   (needs tree-sitter-cli + a C compiler; ~50 parsers)
--   - lsp.lua trimmed to yaml/bash/docker/json/lua
...
-- No wl-clipboard in the container (no Wayland), so leave the system
-- clipboard alone -- yanking to a nonexistent provider just errors.
